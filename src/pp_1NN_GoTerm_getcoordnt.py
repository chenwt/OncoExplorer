...
def getAffinityDW_sga(wl,nw):
    ...
    W = np.zeros([rg_gid, 40], float)
    path = '/usr1/public/yifeng/Github/deepwalk-master/result2/brca_km40_ss_wl'+str(wl)+'_nw'+str(nw)+'.embeddings'
    f = open(path, 'r')
    next(f)
    k = 0
    for line in f:
        ...
        #for cancer in ['brca']:
            for wl in [1,2,3,5,10,20,40]:
                for nw in [10,20,50,100,200]:
                    ...
                    sga2sgaAaff = getAffinityDW_sga(wl,nw)
                    avgacc = 0
                    for trial in range(10):
                        count_total = 0
                        count_true = 0
...
#TODO:
def save_getpaircoor(gene2goId, set_pathway, choice, pathout):
    if choice == 'Baseline1':
        print choice
        for cancer in ['pancan', 'brca', 'gbm', 'ov']:
        #for cancer in ['brca']:
            ...
            path = pathout+'/'+cancer+'_Baseline1.txt'
            f = open(path, 'w')
            for sga in sga2sgaAaff.keys():
                line = sga2sgaAaff[sga]
                for l in line:
                    sga2 = l[0]
                    aff = l[1]
                    if sga2 == sga: continue
                    print >> f, sga+'\t'+sga2+'\t'+str(aff)
    ...
    if choice == 'Lu affinity':
        ...
        for cancer in ['pancan', 'brca', 'gbm', 'ov']:
            ...
            path = pathout+'/'+cancer+'_LuAff.txt'
            f = open(path, 'w')
            for sga in sga2sgaAaff.keys():
                line = sga2sgaAaff[sga]
                for l in line:
    ...
    if choice == 'Proppr':
        ...
        for cancer in ['pancan', 'brca', 'gbm', 'ov']:
            ...
            path = pathout+'/'+cancer+'_Propprmax.txt'
            f = open(path, 'w')
            for sga in sga2sgaAaff.keys():
                line = sga2sgaAaff[sga]
                for l in line:
    ...
    if choice in ['WeightedOverlap_min', 'WeightedOverlap_max', 'WeightedOverlap_prod','WeightedOverlap_avg']:
        ...
        for cancer in ['pancan', 'brca', 'gbm', 'ov']:
            ...
            path = pathout+'/'+cancer+'_'+choice+'.txt'
            f = open(path, 'w')
            for sga in sga2sgaAaff.keys():
                line = sga2sgaAaff[sga]
                for l in line:
    ...
    if choice == 'Jaccard1':
        ...
        #for cancer in ['brca']:
            ...
            path = pathout+'/'+cancer+'_Jaccard.txt'
            f = open(path, 'w')
            for sga in sga2sgaAaff.keys():
                line = sga2sgaAaff[sga]
                for l in line:
    ...
    if choice == 'Lu dist':
        print choice
        for cancer in ['pancan', 'brca', 'gbm', 'ov']:
            sga2sgaAaff = getCoordinates(cancer)

            path = pathout+'/'+cancer+'_LuDist.txt'
            f = open(path, 'w')
            for sga in sga2sgaAaff.keys():
                line = sga2sgaAaff[sga]
                for l in line:
    ...
    if choice == 'DW_sga':
        ...
        for wl in [1,2,3,5,10,20,40]:
            for nw in [10,20,50,100,200]:
                sga2sgaAaff = getAffinityDW_sga(wl,nw)
                path = pathout+'/'+cancer+'_DWsga_wl'+str(wl)+'_nw'+str(nw)+'.txt'
                f = open(path, 'w')
                for sga in sga2sgaAaff.keys():
                    line = sga2sgaAaff[sga]
                    for l in line:
    ...
    if choice == 'DW':
        ...
        for wl in [1,2,3,5,10,20,40]:
            for nw in [10,20,50,100,200]:
                ...
                sga2sgaAaff = getAffinityDW_sga(wl,nw)
                path = pathout+'/'+cancer+'_DW_wl'+str(wl)+'_nw'+str(nw)+'.txt'
                f = open(path, 'w')
                for sga in sga2sgaAaff.keys():
                    line = sga2sgaAaff[sga]
                    for l in line:
...
#    for choice in ['Baseline1', 'Lu affinity', 'Proppr', \
#        'WeightedOverlap_min', 'WeightedOverlap_max', 'WeightedOverlap_prod', \
#        'WeightedOverlap_avg','TFIDF_min','TFIDF_max','TFIDF_avg','TFIDF_prod', \
#        'Jaccard1', 'Lu dist', 'DW_sga']:
#    for metric in ['one']:
    for choice in ['DW']:
        pathout = 'pairdist'
        save_getpaircoor(gene2goId, set_pathway, choice, pathout)
