...
def step2(path_outputData):
    ...
    sga2deg_train_clean = dd(float)
    sga2deg_test_clean = dd(float)
    sga2deg_remain_clean = dd(float)
    ...
    for line in open(path_sga2deg_all, 'r'):
        ...
        if patid in patid_train:
            ...
            if sga2deg_train[(sga,deg)] == 0:
                sga2deg_train_clean[(sga,deg)] = 1.0
            sga2deg_train_clean[(sga,deg)] *= (1.0-prob)
            sga2deg_train[(sga,deg)] += prob
        elif patid in patid_test:
            sga_test_corpus.add(sga)
            deg_test_corpus.add(deg)
            if sga2deg_test[(sga,deg)] == 0:
                sga2deg_test_clean[(sga,deg)] = 1.0
            sga2deg_test_clean[(sga,deg)] *= (1.0-prob)
            sga2deg_test[(sga,deg)] += prob
        elif patid in patid_remain:
            sga_remain_corpus.add(sga)
            deg_remain_corpus.add(deg)
            if sga2deg_remain[(sga2deg)] == 0:
                sga2deg_remain_clean[(sga,deg)] = 1.0
            sga2deg_remain_clean[(sga,deg)] *= (1.0-prob)
